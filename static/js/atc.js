...
var ready=function(e,t){function u(){
	if(!n){
	if(!t.body)return setTimeout(u,13);n=true;if(i){
		var e,r=0;while(e=i[r++])e.call(null);i=null
	}
}
}
function a(){
	...
	{if(t.attachEvent){
		t.attachEvent("onreadystatechange",s);e.attachEvent("onload",u);
		var n=false;
		try{
			n=e.frameElement==null}catch(i){}
			if(b.doScroll&&n)f()
		}
	else{o=e.onload;e.onload=function(e){
		o(e);u()
		}
	}
	}
...
function f(){
	if(n)return;try{b.doScroll("left")}catch(e){setTimeout(f,1);return}u()
	}
	var n=false,r=false,i=[],s,o;if(t.addEventListener){
		s=function(){
			t.removeEventListener("DOMContentLoaded",s,false);u()
		}
	}
	else
		{
			if(t.attachEvent){s=function(){
				if(t.readyState==="complete"){
					t.detachEvent("onreadystatechange",s);u()
				}
			}
		}
	}
...
addtocalendar.languages={de:"In den Kalender",en:"Add to Calendar",es:"AÃ±adir al Calendario",fr:"Ajouter au calendrier",hi:"à¤•à¥ˆà¤²à¥‡à¤‚à¤¡à¤° à¤®à¥‡à¤‚ à¤œà¥‹à¤¡à¤¼à¥‡à¤‚","in":"Tambahkan ke Kalender",ja:"ã‚«ãƒ¬ãƒ³ãƒ€ãƒ¼ã«è¿½åŠ ",ko:"ìº˜ë¦°ë”ì— ì¶”ê°€",pt:"Adicionar ao calendÃ¡rio",ru:"Ð”Ð¾Ð±Ð°Ð²Ð¸Ñ‚ÑŒ Ð² ÐºÐ°Ð»ÐµÐ½Ð´Ð°Ñ€ÑŒ",uk:"Ð”Ð¾Ð´Ð°Ñ‚Ð¸ Ð² ÐºÐ°Ð»ÐµÐ½Ð´Ð°Ñ€",zh:"æ·»åŠ åˆ°æ—¥åŽ†"};
addtocalendar.calendar_urls={};
addtocalendar.loadSettings=function(element){
	var settings={language:"auto","show-list-on":"click",
	calendars:["iCalendar","Google Calendar","Outlook","Outlook Online","Yahoo! Calendar"],
	secure:"auto","on-button-click":function(){},"on-calendar-click":function(){}};
	for(var option in settings){var pname="data-"+option;var eattr=element.getAttribute(pname);
		if(eattr!=null){if(isArray(settings[option])){settings[option]=eattr.replace(/\s*,\s*/g,",").replace(/^\s+|\s+$/g,"").split(",");continue}if(isFunc(settings[option])){
			var fn=window[eattr];
			if(isFunc(fn)){
				settings[option]=fn
			}
...
addtocalendar.load=function(){
	ready(function(){
		...
		var t=-(new Date).getTimezoneOffset().toString();var n=addtocalendar.languages;
		var remTags=document.getElementById("cal-reminder");
		var r=remTags.getElementsByTagName("*"); 
		console.log(r); 
...
addtocalendar.load()})(window,document)
